...
*/
	...
	public class Options {
		 ...
		 */	
		 [CCode (cname ="tox_options_default")]
	...
	public class Tox {
		...
		*/
		...
		public uint8[] get_public_key(){
			uint8[] add = new uint8[ToxCore.PUBLIC_KEY_SIZE];
		...
		public uint8[] get_secret_key(){
			uint8[] add= new uint8[ToxCore.SECRET_KEY_SIZE];
		...
		public void set_name(string name) requires(name.data.length <= ToxCore.MAX_NAME_LENGTH){
			...
			self_set_name(name.data, out err);	
		}
		...
		public bool set_status_message(string status_message) requires(status_message.data.length <= MAX_STATUS_MESSAGE_LENGTH){
			TOX_ERR_SET_INFO err;
			return self_set_status_message(status_message.data,out err);
		}
		/**
		 ...
		 * @return true on success.
		 */
		private bool friend_get_public_key(uint32 friend_number, [CCode (array_length = false)]uint8[] public_key, out TOX_ERR_FRIEND_DELETE error);
		...
		public delegate void FriendStatusFunc(Tox tox, uint32 friend_number, UserStatus status);
		...
		public bool set_typing_for_friend(uint32 friend_number, bool typing){
			TOX_ERR_FRIEND_DELETE err;
			return self_set_typing(friend_number, typing, out err);
		...
		public uint32 send_message_to_friend(uint32 friend_number, MessageType type, string message) throws SendMessageError
			...
			switch(err){
				...
				case TOX_ERR_FRIEND_SEND_MESSAGE.FRIEND_NOT_CONNECTED:
					throw new SendMessageError.FRIEND_NOT_CONNECTED("Friend number "+friend_number.to_string()+ " not connected, maybe try to queue it for later?");
					break;
				case TOX_ERR_FRIEND_SEND_MESSAGE.SENDQ:
					throw new SendMessageError.SENDQ("Error increasing the message queue size for friend number" + friend_number.to_string());
				default:
					break;
			}
			return retval;
		...
		public void file_control(uint32 friend_number, uint32 file_number, FileControlStatus status) throws FileControlError{
			TOX_ERR_FILE_CONTROL err;
			bool res = _file_control(friend_number, file_number, status, out err);
			if(!res){
				switch (err){
					case TOX_ERR_FILE_CONTROL.FRIEND_NOT_FOUND:
						throw new FileControlError.FRIEND_NOT_FOUND("Friend number "+friend_number.to_string()+ " not found");
						break;
					case TOX_ERR_FILE_CONTROL.FRIEND_NOT_CONNECTED:
						throw new FileControlError.FRIEND_NOT_CONNECTED("Friend number " + friend_number.to_string() + " is not connected");
						break;
					case TOX_ERR_FILE_CONTROL.NOT_FOUND:
						throw new FileControlError.NOT_FOUND("File transfer ID "+ file_number.to_string() + " not found");
						break;
					case TOX_ERR_FILE_CONTROL.NOT_PAUSED:
						throw new FileControlError.NOT_PAUSED("File transter ID "+ file_number.to_string() + " not found");
						break;
					case TOX_ERR_FILE_CONTROL.DENIED:
						throw new FileControlError.DENIED("File transfer ID" +file_number.to_string()+ " was paused by the other party");
						break;
					case TOX_ERR_FILE_CONTROL.ALREADY_PAUSED:
						throw new FileControlError.ALREADY_PAUSED("File transfer ID "+file_number.to_string()+ " was already paused");
						break;
		...
		public void file_seek(uint32 friend_number, uint32 file_number, uint64 position) throws FileSeekError{
			...
			bool res = _file_seek(friend_number, file_number, position, out err);
			if (!res){
				switch (err){
					case TOX_ERR_FILE_SEEK.FRIEND_NOT_FOUND:
						throw new FileSeekError.FRIEND_NOT_FOUND("Friend number "+friend_number.to_string()+" was not found");
						break;
			...
			if(!res){
				switch(err){
					case TOX_ERR_FILE_GET.FRIEND_NOT_FOUND:
						throw new FileGetError.FRIEND_NOT_FOUND("Friend number " + friend_number.to_string() + " not found");
						break;
					case TOX_ERR_FILE_GET.NOT_FOUND:
						throw new FileGetError.NOT_FOUND("File number "+file_number.to_string() + " not valid");
						break;
					default:
						break;
				}
			...
			if(res==uint32.MAX){
				switch (err){
					case TOX_ERR_FILE_SEND.FRIEND_NOT_FOUND:
						throw new FileSendError.FRIEND_NOT_FOUND("Friend number" + friend_number.to_string() + " not found");
						break;
					case TOX_ERR_FILE_SEND.FRIEND_NOT_CONNECTED:
						throw new FileSendError.FRIEND_NOT_CONNECTED("Friend number "+friend_number.to_string()+" is not online");
						break;
					case TOX_ERR_FILE_SEND.TOO_MANY:
						throw new FileSendError.TOO_MANY("Too many outgoing connections for friend number " + friend_number.to_string());
						break;
					default:
						break;
				}
		...
		public void send_file_chunk(uint32 friend_number, uint32 file_number, uint64 position, uint8[] data) throws FileSendChunkError{
			...
			bool res = file_send_chunk(friend_number, file_number, position, data, out err);
			if(!res){
				switch (err){
					case TOX_ERR_FILE_SEND_CHUNK.FRIEND_NOT_FOUND:
						throw new FileSendChunkError.FRIEND_NOT_FOUND("Friend number "+friend_number.to_string()+" is not valid");
						break;
					case TOX_ERR_FILE_SEND_CHUNK.FRIEND_NOT_CONNECTED:
						throw new FileSendChunkError.FRIEND_NOT_CONNECTED("Friend number "+friend_number.to_string()+" is not online");
						break;
					case TOX_ERR_FILE_SEND_CHUNK.NOT_FOUND:
						throw new FileSendChunkError.NOT_FOUND("File number "+file_number.to_string()+" not found");
						break;
					case TOX_ERR_FILE_SEND_CHUNK.NOT_TRANSFERRING:
						throw new FileSendChunkError.NOT_TRANSFERRING("Friend number "+friend_number.to_string()+" is not acccepting the connection, check "+file_number.to_string()+" file number");
						break;
					case TOX_ERR_FILE_SEND_CHUNK.INVALID_LENGTH:
						throw new FileSendChunkError.INVALID_LENGTH("Length" + data.length.to_string() +" is not valid for file number "+file_number.to_string());
						break;
					case TOX_ERR_FILE_SEND_CHUNK.SENDQ:
						throw new FileSendChunkError.SENDQ("Packet queue full for file number "+file_number.to_string()+" and friend number "+friend_number.to_string());
						break;
					case TOX_ERR_FILE_SEND_CHUNK.WRONG_POSITION:
						throw new FileSendChunkError.WRONG_POSITION("File number "+file_number.to_string()+" is not in position "+position.to_string());
						break;
					default:
						break;
				}
