...
SENDQ,
		...
		protected enum TOX_ERR_BOOTSTRAP {
 ...
 ******************************************************************************/
	...
	public class Options {
		...
		public static Options? create() throws FileError{
			TOX_ERR_OPTIONS_NEW en;
			Options ret = tox_options_new(en);
			if(en = TOX_ERR_OPTIONS_NEW.MALLOC){
				throw new FileError.NOMEM("Unable to allocate memory for Tox.Options object");
			}
			return ret;
		}
	...
	public class Tox {
		...
		*/
		...
		private bool self_set_name(const uint8[] name, out TOX_ERR_SET_INFO error);
		
		public void set_name(string name)
			requires(name.length <= ToxCore.MAX_NAME_LENGTH + 1){
		...
		private bool connect(string address, uint16 port, [CCode(array_length=false)]const uint8[] public_key, out TOX_ERR_BOOTSTRAP error);
		
		public void bootstrap(string address, uint16 port, const uint8[] public_key) throws BootstrapError{
		...
		private bool _add_tcp_relay(string address, uint16 port, [CCode(array_length=false)]const uint8[] public_key, out TOX_ERR_BOOTSTRAP error);
		[CCode (cname="vala_tox_add_tcp_relay")]
		public void add_tcp_relay(string address, uint16 port, const uint8[] public_key) throws BootstrapError {
			TOX_ERR_BOOTSTRAP err;
		 ...
		 */
		private uint32 friend_add([CCode (array_length=false)] const uint8[] address, const uint8[] message, out TOX_ERR_FRIEND_ADD error);
		public uint32 add_friend(uint8[] address, string message) throws FriendAddError
									requires(message.length <= MAX_FRIEND_REQUEST_LENGTH)
		 ...
		 */
		private uint32 friend_add_norequest([CCode (array_length = false)]const uint8[] public_key, out TOX_ERR_FRIEND_ADD error);
		public uint32 add_friend_norequest(const uint8[] public_key){
			TOX_ERR_FRIEND_ADD err;
		 ...
		 * @return true on success.
		 */
		private bool friend_delete(uint32 friend_number, out TOX_ERR_FRIEND_DELETE);
		 ...
		 */
		private uint32 friend_by_public_key([CCode (array_length = false)]const uint8[] public_key, out TOX_ERR_FRIEND_BY_PUBLIC_KEY error);
		public uint32 get_friend_by_pubkey(const uint8[] public_key) throws FriendGetError{
		...
		public uint8[] get_friend_pubkey(uint32 friend_number) throws FriendGetError{
			int err;
		 ...
		 * @param name A valid memory region large enough to store the friend's name.
		 */
		private bool friend_get_status_message(uint32 friend_number, [CCode (array_length = false)]uint8[] status_message,
   out TOX_ERR_FRIEND_BY_PUBLIC_KEY error);
		...
		public delegate void FriendStatusMessageFunc(Tox tox, uint32 friend_number, const uint8[] message_missing_null);
		...
		public delegate void FriendRequestFunc(Tox tox, [CCode (array_length = false)]const uint8 public_key, const uint8[] message_missing_null);
		...
		public delegate void FriendMessageFunc(Tox tox, uint32 friend_number, MessageType type, const uint8[] message_missing_null);
		 ...
		 */
		private bool hash([CCode (array_length = false)]uint8[] hash, const uint8[] data);
		...
		private bool _file_seek(uint32 friend_number, uint32 file_number, uint64 position, out TOX_ERR_FILE_SEEK error);
		 ...
		 * @return true on success.
		 */
		private bool file_get_file_id(uint32 friend_number, uint32 file_number, [CCode (array_length = false)]uint8[] file_id, out TOX_ERR_FILE_GET error);
